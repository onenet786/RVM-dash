...
- **Solution**: Run `git fetch origin` first so git learns about the new branch, and use `-f` to force checkout over local `.env` edits:
  ```bash
  cd /www/wwwroot/rvm-dash
  git clean -fd dist/ 2>/dev/null || true
  git checkout -f -- .
  git fetch origin
  git checkout -f B4-Postgre-switching
  git pull origin B4-Postgre-switching
  npm run build
  ```
  *Or single-command force update for branch B4-Postgre-switching*:
  ```bash
  cd /www/wwwroot/rvm-dash && git fetch origin && git checkout -f -B B4-Postgre-switching origin/B4-Postgre-switching && git reset --hard origin/B4-Postgre-switching && npm run build
  ```
