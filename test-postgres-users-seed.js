...
async function testPostgresUserSeeding() {
  console.log('--- TESTING POSTGRESQL USER SEEDING & FETCH ---');
  ...
  try {
    ...
    // 2. Fetch Users from PostgreSQL Engine
    const usersRes = await fetch('http://localhost:5009/api/security/users');
    const usersData = await usersRes.json();
    console.log('PostgreSQL Users List Status:', usersRes.status);
    console.log('PostgreSQL Admin Accounts:', JSON.stringify(usersData, null, 2));
    ...
    console.error('PostgreSQL User Seeding Test Error:', err.message);
  }
}

testPostgresUserSeeding();
